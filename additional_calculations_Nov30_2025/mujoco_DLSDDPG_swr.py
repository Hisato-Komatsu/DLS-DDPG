...
#10/15 introduced sampling WITH replacement for DDPG update (but not for LR update)
...
class Agent:
   ...
   def LS_replay(self, replay_actor, replay_critic, weight_a, sample_size):
      s_sample, a_sample, r_sample, next_s_sample, done_sample = self.net.memory_sample(sample_size = sample_size, replacement=False)
...
class Whole_Net():
   ...
   def memory_sample(self, sample_size, replacement):
      if replacement: # for DDPG update
   ...
   def DDPG_train(self, train_actor=False, train_critic=False):
      ...
      s_sample, a_sample, r_sample, next_s_sample, done_sample = self.memory_sample(sample_size = self.minibatch_size, replacement=True)
      if train_critic:
         with torch.no_grad():
...
if __name__ == '__main__':
   ...
   name_condition = "_LS_" + str(int(LS_a)) + str(int(LS_c)) + "_DDPG_" + str(int(DDPG_a)) + str(int(DDPG_c)) + "_BOAC_" + str(int(hyper.optim_choosing)) + "_game_" + game + "_gamma_" + str(hyper.gamma) + "_Nl_" + str(hyper.layer_size) + "_betaaQ_" + str(hyper.beta_a) + "_" + str(hyper.beta_Q) + "_to_" + str(hyper.beta_a_min) + "_" + str(hyper.beta_Q_min) + "_weighta_" + str(hyper.weight_actor) + "_upb_" + str(hyper.restrict_upb) + "_resid_" + str(hyper.resid_coef) + "_SSs_" + str(int(start_steps)) + "_" + str(seed) + "th_try"

   filename_learning_curve = "LC_DLSDDPGswr_softT_offC" + name_condition + ".txt"
   file_learning_curve = open(filename_learning_curve, "w")
   
   filename_learning_curve_eval = "LC_eval_DLSDDPGswr_softT_offC" + name_condition + ".txt"
   file_learning_curve_eval = open(filename_learning_curve_eval, "w")
   
   weight_record = hyper.weight_record
   if weight_record:
      filename_weight = "weight_DLSDDPGswr_softT_offC" + name_condition + ".txt"
   ...
   if hyper.time_record:
      filename_time = "time_DLSDDPGswr_softT_offC" + name_condition + ".txt"
   ...
   if hyper.movie_record:
      ...
      name_prefix = "movie_DLSDDPGswr" + name_condition
