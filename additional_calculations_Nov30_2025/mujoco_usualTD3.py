...
class Whole_Net():
   ...
   def target_q(self, x, add_rand):
      ...
      if add_rand:
         ...
         mut = torch.clip( mut+mut_rand, -1.0, 1.0)
   ...
   for trial in range (10):
      ...
      while not (d0 or trunc):
         if (gtime) <= rds :
            ...
            with torch.no_grad():
               a0, _ = (ag.forward_actor( s0[None,:], add_rand=False ))
...
if __name__ == '__main__':
   ...
   filename_learning_curve = "LC_TD3swr" + name_condition + ".txt"
   file_learning_curve = open(filename_learning_curve, "w")
   
   filename_learning_curve_eval = "LC_eval_TD3swr" + name_condition + ".txt"
   ...
   while global_time < 1000000 :
      ...
      while not done:
         ...
         if (global_time+1) <= random_steps :
            ...
            with torch.no_grad():
               action, action_scaled = agent.forward_actor( state[None,:], add_rand=True )
   ...
   if hyper.time_record:
      filename_time = "time_TD3swr" + name_condition + ".txt"
   ...
   if hyper.movie_record:
      ...
      name_prefix = "movie_TD3swr" + name_condition
      ...
      while not done:
         ...
         with torch.no_grad():
            action, _ = (agent.forward_actor( state[None,:], add_rand=False ))
            action = action[0].detach().numpy()
         next_state, reward, terminated, truncated, info = env.step(action)
         next_state = torch.from_numpy(next_state)
         done = terminated or truncated
