...
class hyperparameters:
   def __init__(self):
      ...
      self.gamma = 0.99
      self.beta_Q = 1.0e-1 #1.0e-2
      #self.beta_a = 1.0e-2 #1.0e-2
      self.beta_Q_min = 1.0e-2 #1.0e-3
   ...
   for trial in range (10):
      ...
      while not (d0 or trunc):
         if (gtime) <= rds :
            ...
            with torch.no_grad():
               a0 = (ag.net.forward_actor( s0[None,:], add_rand=False ))[0]
               a0 = a0.detach().numpy()
...
if __name__ == '__main__':
   ...
   filename_learning_curve = "LC_LScDDPGswr_softT_offC" + name_condition + ".txt"
   file_learning_curve = open(filename_learning_curve, "w")
   
   filename_learning_curve_eval = "LC_eval_LScDDPGswr_softT_offC" + name_condition + ".txt"
   file_learning_curve_eval = open(filename_learning_curve_eval, "w")
   
   weight_record = hyper.weight_record
   if weight_record:
      filename_weight = "weight_LScDDPGswr_softT_offC" + name_condition + ".txt"
   ...
   while global_time < 1000000 :
      ...
      while not done:
         ...
         if (global_time+1) <= random_steps :
            ...
            with torch.no_grad():
               action = (agent.net.forward_actor( state[None,:], add_rand=True ))[0]
   ...
   if hyper.time_record:
      filename_time = "time_LScDDPGswr_softT_offC" + name_condition + ".txt"
   ...
   if hyper.movie_record:
      ...
      name_prefix = "movie_LScDDPGswr" + name_condition
      ...
      while not done:
         ...
         with torch.no_grad():
            action = (agent.net.forward_actor( state[None,:], add_rand=False ))[0]
            action = action.detach().numpy()
         next_state, reward, terminated, truncated, info = env.step(action)
         next_state = torch.from_numpy(next_state)
         done = terminated or truncated
