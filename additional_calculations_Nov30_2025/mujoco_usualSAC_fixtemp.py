...
class Whole_Net():
   ...
   def target_q(self, x, add_rand=True):
      with torch.no_grad():
         ...
         qt_w_ent = torch.minimum( self.target_net_critic_A.fc2(vt_A).squeeze(1) , self.target_net_critic_B.fc2(vt_B).squeeze(1) ) - self.alpha*log_pt
   ...
   def SAC_train(self, train_actor=False, train_critic=False):
      ...
      if train_actor:
         ...
         loss_actor = torch.mean( - main_q_actor + self.alpha*log_prob ) 
...
if __name__ == '__main__':
   ...
   filename_learning_curve = "LC_SACswr_fixtemp" + name_condition + ".txt"
   file_learning_curve = open(filename_learning_curve, "w")
   
   filename_learning_curve_eval = "LC_eval_SACswr_fixtemp" + name_condition + ".txt"
   ...
   if hyper.time_record:
      filename_time = "time_SACswr_fixtemp" + name_condition + ".txt"
   ...
   if hyper.movie_record:
      ...
      name_prefix = "movie_SACswr_fixtemp" + name_condition
